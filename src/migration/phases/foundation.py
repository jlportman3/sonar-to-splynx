...
def _apply_roles_to_splynx(normalized_roles: Iterable[NormalizedRole]) -> Dict[str, str]:
...
def _apply_permissions_to_splynx(
    ...
    # Clear existing rules for roles we manage
    for slug in role_map.values():
        _run_mysql_query(f"DELETE FROM permissions_role WHERE id='{slug}';")
    ...
    for assignment in assignments:
        ...
        for rule in target_rules:
            ...
            _run_mysql_query(
                "INSERT INTO permissions_role (id, controller, module, action, rule) "
                f"VALUES ('{role_slug}', '{controller}', '{module}', '{action}', '{rule_value}');"
            )
            inserted += 1

    logger.info(f"Applied permissions to Splynx: inserted_rules={inserted}, skipped_assignments={skipped}")
...
def normalize_roles(output_path: Optional[str] = None, apply_to_splynx: bool = False) -> None:
    ...
    try:
        ...
        if apply_to_splynx:
            role_slug_map = _apply_roles_to_splynx(normalized_roles)
