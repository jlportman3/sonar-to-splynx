...
def main():
    ap = argparse.ArgumentParser(description="Build ticket_groups_out.csv from Sonar ticket_groups.csv")
    ap.add_argument("--in", dest="infile", required=True, help="Input ticket_groups.csv")
    ap.add_argument("--out", dest="outfile", required=True, help="Output ticket_groups_out.csv")
    args = ap.parse_args()

    df = read_csv_auto(args.infile)
    col_id = resolve_col(df, "ID", ["id"])
    col_name = resolve_col(df, "name", ["Name", "title", "group name"])
